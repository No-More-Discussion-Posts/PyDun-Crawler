...
def test_monster_movement(game):
    ...
    distance = sqrt((player.get(Position).x-monster.x)**2+(player.get(Position).y-monster.y)**2)
    monster.movement()
    new_distance = sqrt((player.get(Position).x-monster.x)**2+(player.get(Position).y-monster.y)**2)
